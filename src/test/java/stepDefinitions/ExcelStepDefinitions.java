...
public class ExcelStepDefinitions {
    ...
    public void satirdaki_hucreyi_yazdirir(Integer satir, Integer sutun) {
        String istenenHucreYazisi=workbook.getSheet("Sayfa1")
                                  ...
                                  .getCell(sutun-1)
                                  .toString();
        System.out.println(satir+" satir "+sutun+" sutundaki değer :"+istenenHucreYazisi);
    ...
    public void ulke_sayisinin_oldugunu_test_eder(Integer ulkeSayisi) {
        int actualUkleSayisi=workbook.getSheet("Sayfa1").getLastRowNum();
        Assert.assertTrue(ulkeSayisi==actualUkleSayisi);
    }


    @Then("Fiziki olarak kullanilan satir sayisinin {int} oldugunu test eder")
    public void fiziki_olarak_kullanilan_satir_sayisinin_oldugunu_test_eder(Integer fizikiKullanılanSatir) {

        int actualFizikiKullanılanSatir=workbook.getSheet("Sayfa1").getPhysicalNumberOfRows();
        Assert.assertTrue(fizikiKullanılanSatir==actualFizikiKullanılanSatir);
